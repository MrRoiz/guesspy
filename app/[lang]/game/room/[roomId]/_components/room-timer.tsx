...
export const RoomTimer: FC<RoomTimerProps> = ({
  ...
  const isFinished = isExpired || isTimerStopped;
        ...
        <div
          className={cn(
            ...
            getTimerColor(),
            {
              ...
              'border-destructive':
                (timeLeft <= 30 && !isFinished) || isTimerStopped,
        ...
        <div className="flex flex-col items-center gap-4">
          {showSpies && (
            <div className="rounded-lg border bg-card p-6 shadow-lg">
              ...
              <ul className="space-y-2">
                {spies.map((spy) => (
                  <li key={spy.id} className="text-destructive text-lg">
                    🕵 {spy.name}
                  </li>
                ))}
              </ul>
          ...
          {!isHost && (
            <p className="text-center text-muted-foreground text-sm">
              {dict.room.waitingForHostToRestart}
            </p>
          )}
        </div>
      )}
